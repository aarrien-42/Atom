...
file_count=$(ls $directory/11_code_examples/test_*.atm 2>/dev/null | wc -l)
...
do
    ...
    num=$(printf "%02d" $i)
    test_file="$directory/11_code_examples/test_${num}.atm"
    
    # Check if the test file exists
    if [ -f "$test_file" ]; then
        echo "Running $test_file..."
        ./bin/atom $test_file > $directory/13_actual_results/test_$num.log
    else
